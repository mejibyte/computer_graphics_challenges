...
RgbImage::RgbImage( int numRows, int numCols )
{
	...
	if ( !ImagePtr ) {
		fprintf(stderr, "Unable to allocate memory for %d x %d bitmap.\n", 
				NumRows, NumCols);
		Reset();
...
{  
	...
	ImagePtr = new unsigned char[NumRows*GetNumBytesPerRow()];
	if ( !ImagePtr ) {
		fprintf(stderr, "Unable to allocate memory for %d x %d bitmap: %s.\n", 
...
bool RgbImage::LoadFromOpenglBuffer()					// Load the bitmap from the current OpenGL buffer
{
	...
	if ( ImagePtr==0 ) { // If no memory allocated
		...
		ImagePtr = new unsigned char[NumRows*GetNumBytesPerRow()];
		if ( !ImagePtr ) {
			fprintf(stderr, "Unable to allocate memory for %d x %d buffer.\n", 
					NumRows, NumCols);
			Reset();
			ErrorCode = MemoryError;
			return false;
